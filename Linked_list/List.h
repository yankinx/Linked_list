		...
		if (head != NULL)
		{
			...
			do
			{
				if (temp->value == data)
				{
					...
					//	head = head->next;
					//}
					// было не верно написано! мы уже проверили что значение €вл€етс€ искомым, но рассмотрено только условие что элемент 1, а если их несколько нужно будет только передвинуть голову вперед
					if (head == temp)
					{
						head = head->next;
						if (head == temp) {
